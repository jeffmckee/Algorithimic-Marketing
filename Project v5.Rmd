...
for (p in seq(from = .01, to= 1, by = .01)){
  n<-round(nrow(df_test_profit)*p)
  model_curve_lasso[p*100,2] <- df_test_profit[1:n, sum(Class)]*Success_profit - (n-df_test_profit[1:n, sum(Class)])*fail_cost
}
```
...
for (p in seq(from = .01, to= 1, by = .01)){
  n<-round(nrow(df_test_profit)*p)
  model_curve_logit[p*100,2] <- df_test_profit[1:n, sum(Class)]*Success_profit - (n-df_test_profit[1:n, sum(Class)])*fail_cost
}
...
for (p in seq(from = .01, to= 1, by = .01)){
  n<-round(nrow(df_test_profit)*p)
  model_curve_random[p*100,2] <- df_test_profit[1:n, sum(Class)]*Success_profit - (n-df_test_profit[1:n, sum(Class)])*fail_cost
}
