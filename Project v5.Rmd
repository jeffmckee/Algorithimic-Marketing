...
model_df<-full_df[campaign_no<=5,c(56,1:13,59:62)]
x_matrix <- model.matrix(Class~.+.*campaign2+.*campaign3+.*campaign4+.*campaign5-1,data=model_df)

```

```{r}
cv_lambda <- cv.glmnet(x=x_matrix, y=model_df$Class, family="binomial")
best_lambda <- cv_lambda$lambda.min
enet <- glmnet(x=x_matrix, y=model_df$Class, family="binomial")
plot(enet)
summary(predict(enet, newx=x_matrix, s=best_lambda,type="response"))
...
sub_df <- full_df[1:50000,]

# campaign dummies; age; retired; student. adjust to match other models
TrnX <- sub_df[campaign_no <=5,c(58:61,1,17,23)]

OrigTrnGdf <- sub_df[campaign_no <=5,54]
OrigTrnG <- OrigTrnGdf$Class

sub_TstX <- full_df[50001:60000]
TstX <- sub_TstX[campaign_no <=5,c(58:61,1,17,23)]

for (i in 1:20){
  nn <- knn(TrnX, TstX, OrigTrnG, k=i, l = 0, prob=FALSE, use.all = TRUE)
  print(summary(nn))
}

### KNN - euclidean approach using non-blown-out data
TrnX <- df_train[df_train$campaign <=5,c(1,17,23)]
OrigTrnG <- df_train[df_train$campaign <=5,54]

TstX <- df_test[df_test$campaign <=5,c(1,17,23)]
...
### KNN stuff that Verdon couldn't get to work
## first Mahalanobis approach: knnGarden package
#install.packages("knnGarden")
library(knnGarden)
knnMCN(TrnX=TrnX,OrigTrnG=OrigTrnG,TstX=TstX,ShowObs=FALSE,K=1)

## second Mahalanobis approach: pvclass package. computationally taxing; subsetted to 1000 rows in dev 
library(pvclass)
knnout <- cvpvs.knn(TrnX, OrigTrnG, k = 2, distance = 'mahalanobis', cova = 'standard')
```