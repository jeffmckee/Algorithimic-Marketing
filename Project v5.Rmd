...
```{r}
# cv_enet <- cv.glmnet(x=x_matrix, y=model_df$Class, family="binomial",alpha=0)
# best_lambda <- cv_enet$lambda.min
best_lambda <- 0.00673510701979613
enet <- glmnet(x=x_matrix, y=model_df$Class, family="binomial",alpha=0)

```

```{r}
test_reg   <-glm(Class~(age+jobadmin.+jobbluecollar+jobentrepreneur+jobhousemaid+jobmanagement+jobretired+jobselfemployed+jobservices+jobstudent+jobtechnician+jobunemployed+jobunknown) *( campaign2+campaign3)+campaign4+campaign5,family='binomial',data=model_df)
summary(test_reg)
```
...
for (x in 1:nrow(df_test)) {
  df_test$Max_calls_lasso[x] <-Compute_Sample_Profitability_LASSO(x)
  }
...
barplot(c(baseline_profit,two_call_profit,three_call_profit,lasso_profit))
