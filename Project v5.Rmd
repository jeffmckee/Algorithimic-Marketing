...
df2$Class <- ifelse(df2$y=="true",TRUE,FALSE)
...
df_train2<-df2[train_ind,c(22,1:8,14:15, 12)]
df_test2<-df2[-train_ind,c(22,1:8,14:15, 12)]
...
prob1 <- predict(fit1,newdata=tree_test,type="prob")[,2]
    tree_test$campaign2<-1
prob2 <- predict(fit1,newdata=tree_test,type="prob")[,2]
    tree_test$campaign2<-0
    tree_test$campaign3<-1
prob3 <- predict(fit1,newdata=tree_test,type="prob")[,2]
    tree_test$campaign3<-0
    tree_test$campaign4<-1
prob4 <- predict(fit1,newdata=tree_test,type="prob")[,2]
    tree_test$campaign4<-0
    tree_test$campaign5<-1
prob5 <- predict(fit1,newdata=tree_test,type="prob")[,2]
prob<-cbind(prob1,prob2,prob3,prob4,prob5)

    #For Calls 1 through N, compute likelihood of success by the nth call: p_sub(n)
        
p_sub1 <- prob[,1]
p_sub2 <- p_sub1+prob[,2]*(1-p_sub1)
p_sub3 <- p_sub2+prob[,3]*(1-p_sub2)
p_sub4 <- p_sub3+prob[,4]*(1-p_sub3)
p_sub5 <- p_sub4+prob[,5]*(1-p_sub4)
...
profit1 <- Success_profit*prob1-fail_cost*(1-prob1)
profit2 <- Success_profit*prob2-fail_cost*(1-prob2)
profit3 <- Success_profit*prob3-fail_cost*(1-prob3)
profit4 <- Success_profit*prob4-fail_cost*(1-prob4)
profit5 <- Success_profit*prob5-fail_cost*(1-prob5)
Profit <- cbind(profit1,profit2,profit3,profit4,profit5)

Calls_to_make <-rowSums(Profit>0)
Success_calls <- ifelse(Calls_to_make>=df_test$campaign & df_test$Class,df_test$campaign,0)
Fail_calls <- ifelse(Calls_to_make>=df_test$campaign,ifelse(!df_test$Class,df_test$campaign,df_test$campaign-1),Calls_to_make)
...
( Expected_profit <- -fail_cost*sum(Fail_calls) + Success_profit*sum(Success_calls>0) )
...
for (x in 1:nrow(df_test)) {
  if  (df_test$campaign[x]>Calls_to_make[x]){
      tree_profit<-tree_profit-Calls_to_make[x]*fail_cost 
      } else {
                if(df_test$Class[x]=="FALSE"){ 
                  tree_profit<-tree_profit-df_test$campaign[x]*fail_cost
                } else { 
                    tree_profit<-tree_profit+Success_profit-(df_test$campaign[x]-1)*fail_cost
                    }
              }
